...
//  }
  ...
  // of a road
  if (lane >=0) {
	bool is_carwaiting = lane ? road->is_carwaiting_a : road->is_carwaiting_b;
	if (u<1) {
		if (u>0.9 && is_carwaiting){
			lastV = time;
		} 
		else{
		...
		if (road->is_intersection) {
			...
			if (pause++ < 20 && road->is_stopsign){
				lastV = time;
				if (lane){
					road->is_carwaiting_a = true;
				}
				else{
					road->is_carwaiting_b = true;
				}
			}
			else{
				if (lane) {				// we've arrived at A
					road->is_carwaiting_a = false;
					if (road->endA) {
						road = lastRoad->endA;
						lane = lastRoad->laneA;
					}
