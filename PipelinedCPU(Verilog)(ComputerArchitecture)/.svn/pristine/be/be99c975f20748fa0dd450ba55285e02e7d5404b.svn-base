...
module ID(memHazard, lwStall, src1, src0, brType, zr_en,
	...
	input clk, memReEx, realHlt, weIn, idexMemWeIn, exmemMemWeIn;
	...
	MEM_DETECT    memDetect     (memHazard, p1_addr, dstEx, mem_we, memReEx);
...
module BYPASS_DETECT(mem0, mem1, ex0, ex1, rd1, rd0, re1, re0, dstEx, dstMem, current_mem_we, idexMemWe, exmemMemWe);
	output mem0, mem1, ex0, ex1;
	input [3:0] rd1, rd0, dstEx, dstMem;
	input re1, re0, current_mem_we, idexMemWe, exmemMemWe;
	...
	*/
	...
	assign ex0 = (re0 == 1 && rd0 == dstEx && idexMemWe == 0) ? 1 : 0;
	assign ex1 = (current_mem_we == 0 && re1 == 1 && rd1 == dstEx) ? 1 : 0;
	assign mem0 = (re0 == 1 && rd0 == dstMem && exmemMemWe == 0) ? 1 : 0;
	assign mem1 = (current_mem_we == 0 && re1 == 1 && rd1 == dstMem) ? 1 : 0;
	
endmodule

module MEM_DETECT(memHazard, readPort1, dstNext, memWeCurrent, memReNext);
	output memHazard;
	input [3:0] readPort1, dstNext;
	input memWeCurrent, memReNext;

	assign memHazard = (memWeCurrent==1 && dstNext == readPort1) ? 1 : 0;
endmodule 
